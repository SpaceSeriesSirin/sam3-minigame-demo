...
# 面板系统
C_PANEL_BG = (28, 32, 38)
C_PANEL_HEADER = (38, 44, 54)
C_PANEL_BORDER = (70, 85, 100)
C_PANEL_INNER = (22, 26, 30)

# 军事终端文字色
C_TEXT_PRIMARY = (220, 230, 240)
C_TEXT_SECONDARY = (150, 165, 180)
C_TEXT_DIM = (90, 105, 120)

# 荧光高亮色（仅用于关键数据和CRT屏幕）
C_PHOSPHOR_GREEN = (80, 255, 120)
C_PHOSPHOR_GREEN_DIM = (40, 120, 60)
C_PHOSPHOR_AMBER = (255, 200, 60)
...
C_P18_BG = (8, 6, 3)
C_P18_GRID = (90, 70, 20)
C_P18_SWEEP = (255, 210, 80)
C_P18_BLIP = (255, 230, 120)
C_P18_MISSILE = (255, 180, 80)
C_P18_AFTERGLOW = (200, 160, 50)
...
def draw_panel(surface, rect, title="", title_color=C_TEXT_PRIMARY, header_color=C_PANEL_HEADER):
    """绘制军事终端风格面板 - WARNO风格"""
    x, y, w, h = rect
    header_h = 26
    
    # 面板背景
    pygame.draw.rect(surface, C_PANEL_BG, rect, border_radius=2)
    # 外边框
    pygame.draw.rect(surface, C_PANEL_BORDER, rect, width=1, border_radius=2)
    
    # 标题栏
    if title:
        header_rect = (x, y, w, header_h)
        pygame.draw.rect(surface, header_color, header_rect, border_top_left_radius=2, border_top_right_radius=2)
        # 标题栏底部线
        pygame.draw.line(surface, C_PANEL_BORDER, (x, y + header_h), (x + w, y + header_h), 1)
    
    # 内阴影效果
    pygame.draw.rect(surface, C_PANEL_INNER, (x + 4, y + header_h + 4, w - 8, h - header_h - 8), border_radius=1)
    
    # 标题文字
    if title:
        font = pygame.font.SysFont("consolas", 13, bold=True)
        txt = font.render(title, True, title_color)
        surface.blit(txt, (x + 10, y + 6))

def draw_display_bezel(surface, rect, label=""):
    """绘制CRT显示器边框 - 拟物化"""
    x, y, w, h = rect
    # 外框
    pygame.draw.rect(surface, (15, 18, 22), (x-4, y-4, w+8, h+8), border_radius=3)
    pygame.draw.rect(surface, (60, 70, 80), (x-4, y-4, w+8, h+8), width=2, border_radius=3)
    # 内框
    pygame.draw.rect(surface, (8, 10, 12), (x-1, y-1, w+2, h+2))
    # 屏幕玻璃反光（顶部）
    glare = pygame.Surface((w, 6), pygame.SRCALPHA)
    glare.fill((255, 255, 255, 8))
    surface.blit(glare, (x, y))
    # 标签
    if label:
        font = pygame.font.SysFont("consolas", 11, bold=True)
        lbl = font.render(label, True, C_TEXT_SECONDARY)
        surface.blit(lbl, (x + w//2 - lbl.get_width()//2, y - 18))

def draw_crt_effect(surface, rect, scanline_spacing=2, intensity=0.12, noise_density=2000):
    """CRT磷光屏效果 - 仅用于显示器内部"""
    x, y, w, h = rect
    ...
    if random.random() < 0.25:
        ...
        for _ in range(max(1, (w * h) // noise_density)):
            nx = random.randint(0, w-1)
            ny = random.randint(0, h-1)
            noise_surf.set_at((nx, ny), (255, 255, 255, random.randint(5, 18)))
        surface.blit(noise_surf, (x, y))

def draw_soviet_switch(surface, x, y, label_up, label_down, state_up, font):
    """拨动开关 - 军事终端风格"""
    pygame.draw.rect(surface, C_PANEL_INNER, (x-14, y-20, 28, 40), border_radius=2)
    pygame.draw.rect(surface, C_PANEL_BORDER, (x-14, y-20, 28, 40), width=1, border_radius=2)
    
    handle_y = y - 10 if state_up else y + 10
    pygame.draw.line(surface, C_TEXT_SECONDARY, (x, y), (x, handle_y), 3)
    pygame.draw.circle(surface, C_PHOSPHOR_GREEN if state_up else C_PHOSPHOR_RED, (x, handle_y), 5)
    ...
    surface.blit(lbl_u, (x - lbl_u.get_width()//2, y - 34))
    surface.blit(lbl_d, (x - lbl_d.get_width()//2, y + 24))

def draw_soviet_lamp(surface, x, y, label, color_on, is_lit, font):
    """指示灯 - 军事终端风格"""
    pygame.draw.circle(surface, C_PANEL_INNER, (x, y), 12)
    pygame.draw.circle(surface, C_PANEL_BORDER, (x, y), 12, 1)
    ...
    if is_lit:
        ...
        pygame.draw.circle(glow, (*color[:3], 45), (16, 16), 14)
        surface.blit(glow, (x-16, y-16), special_flags=pygame.BLEND_ADD)
        pygame.draw.circle(surface, (255, 255, 255), (x-2, y-2), 2)
    
    lines = label.split("\n")
    for i, line in enumerate(lines):
        lbl = font.render(line, True, C_TEXT_PRIMARY if is_lit else C_TEXT_DIM)
        surface.blit(lbl, (x - lbl.get_width()//2, y + 16 + i*13))

def draw_soviet_button(surface, x, y, label, is_active, font, radius=22):
    """发射按钮 - 军事终端风格"""
    pygame.draw.circle(surface, C_PANEL_INNER, (x, y), radius+4)
    pygame.draw.circle(surface, C_PANEL_BORDER, (x, y), radius+4, 1)
    
    color = C_PHOSPHOR_RED if is_active else C_PHOSPHOR_RED_DIM
    pygame.draw.circle(surface, color, (x, y), radius)
    pygame.draw.circle(surface, (255, 120, 100) if is_active else C_TEXT_DIM, (x, y), radius, 2)
    
    if is_active:
        pygame.draw.circle(surface, (255, 150, 150), (x-5, y-5), 4)
    
    lbl = font.render(label, True, C_TEXT_PRIMARY)
    surface.blit(lbl, (x - lbl.get_width()//2, y - lbl.get_height()//2))

def draw_dial_gauge(surface, cx, cy, radius, value, max_val, label, font):
    """圆形仪表 - 军事终端风格"""
    pygame.draw.circle(surface, C_PANEL_INNER, (cx, cy), radius+6)
    pygame.draw.circle(surface, C_PANEL_BORDER, (cx, cy), radius, 2)
    ...
    pygame.draw.circle(surface, C_PANEL_BORDER, (cx, cy), 4)
    
    lbl = font.render(label, True, C_TEXT_SECONDARY)
    surface.blit(lbl, (cx - lbl.get_width()//2, cy + 14))
    
    val_txt = font.render(f"{int(value)}", True, C_TEXT_PRIMARY)
    surface.blit(val_txt, (cx - val_txt.get_width()//2, cy + 28))

def draw_military_grid(surface, width, height, spacing=40):
    """绘制军用网格背景"""
...
class SA3Simulator:
    ...
    def draw(self):
        ...
        # 军用网格背景（仅在模拟和菜单状态）
        if self.state in [STATE_MENU, STATE_SIMULATION]:
            draw_military_grid(self.screen, SCREEN_WIDTH, SCREEN_HEIGHT, 50)
    ...
    def draw_cockpit(self):
        self.draw_panel_p18(10, 10, 300, 430)
        self.draw_panel_fcr(320, 10, 950, 560)
        self.draw_panel_launch(1280, 10, 310, 560)
        self.draw_panel_controls(10, 450, 300, 440)
        self.draw_panel_status(320, 580, 950, 310)

    def draw_panel_p18(self, x, y, w, h):
        """P-18预警雷达面板"""
        draw_panel(self.screen, (x, y, w, h), "P-18 ACQUISITION RADAR", C_PHOSPHOR_AMBER)
        
        header_h = 26
        cx, cy = x + w//2, y + header_h + 210
        radius = 130
        
        # CRT显示器边框
        draw_display_bezel(self.screen, (cx-radius, cy-radius, radius*2, radius*2), "PPI SCOPE")
    ...
    def draw_panel_fcr(self, x, y, w, h):
        """火控雷达主面板"""
        draw_panel(self.screen, (x, y, w, h), "SNR-125M1 FIRE CONTROL", C_PHOSPHOR_GREEN)
        
        header_h = 26
        # TV光学显示器
        self.draw_tv_display(x + 340, y + header_h + 10, 340, 270)
        
        # И1显示器
        self.draw_scope_i1(x + 15, y + header_h + 10, 180, 350)
        
        # И2显示器
        self.draw_scope_i2(x + 730, y + header_h + 10, 180, 350)
        
        # 仪表行
        gauge_y = y + header_h + 380
        
        draw_dial_gauge(self.screen, x + 90, gauge_y + 50, 42, self.ant_el, 90, "ELEV", self.font_sm)
        draw_dial_gauge(self.screen, x + 200, gauge_y + 50, 42, self.range_gate/1000, self.max_range/1000, "RNG km", self.font_sm)
        
        radial_v = self.target.get_radial_velocity() if self.target.active else 0
        draw_dial_gauge(self.screen, x + 310, gauge_y + 50, 42, max(0, radial_v), 700, "Vr m/s", self.font_sm)
        
        draw_dial_gauge(self.screen, x + 420, gauge_y + 50, 42, self.track_quality * 100, 100, "QUAL %", self.font_sm)
        
        # 开关行
        sw_y = gauge_y + 55
        sw_start = x + 520
        
        draw_soviet_switch(self.screen, sw_start, sw_y, "80", "40", self.sw_range_mode == 0, self.font_sm)
        draw_soviet_switch(self.screen, sw_start + 60, sw_y, "MTI", "---", self.sw_sdc, self.font_sm)
        draw_soviet_switch(self.screen, sw_start + 120, sw_y, "PS", "TT", self.sw_guidance == GUIDANCE_LEAD, self.font_sm)
        draw_soviet_switch(self.screen, sw_start + 180, sw_y, "WIDE", "NAR", self.sw_tv_fov == 0, self.font_sm)
        
        # 指示灯
        lamp_y = gauge_y + 55
        lamp_start = x + 740
        
        draw_soviet_lamp(self.screen, lamp_start, lamp_y, "LOCK", C_LAMP_GREEN, self.locked, self.font_sm)
        draw_soviet_lamp(self.screen, lamp_start + 50, lamp_y, "TRK", C_LAMP_GREEN, self.track_quality > 0.5, self.font_sm)
        draw_soviet_lamp(self.screen, lamp_start + 100, lamp_y, "AUTO", C_LAMP_GREEN, self.locked and self.pwr_hv, self.font_sm)
        draw_soviet_lamp(self.screen, lamp_start + 150, lamp_y, "AUTH", C_LAMP_YELLOW, self.check_launch_envelope() and self.locked, self.font_sm)

    def draw_scope_i1(self, x, y, w, h):
        """И1显示器 - 精细跟踪"""
        draw_display_bezel(self.screen, (x, y, w, h), "I1 PRECISION")
    ...
    def draw_scope_i2(self, x, y, w, h):
        """И2显示器 - 搜索"""
        draw_display_bezel(self.screen, (x, y, w, h), f"I2 {self.max_range//1000}KM")
        pygame.draw.rect(self.screen, C_FCR_BG, (x, y, w, h))
    ...
    def draw_tv_display(self, x, y, w, h):
        """TV光学显示器 - 绿色夜视CRT"""
        draw_display_bezel(self.screen, (x, y, w, h), "TOV 9Sh33A")
    ...
    def draw_panel_launch(self, x, y, w, h):
        """发射控制面板"""
        draw_panel(self.screen, (x, y, w, h), "UK-31M1 LAUNCH CONTROL", C_PHOSPHOR_RED)
        
        header_h = 26
        grid_x = x + 45
        grid_y = y + header_h + 20
        cell_w = 58
        cell_h = 26
        
        headers = ["I", "II", "III", "IV"]
        for i, hdr in enumerate(headers):
            txt = self.font_sm.render(hdr, True, C_TEXT_SECONDARY)
        ...
        total_y = y + header_h + 130
        pygame.draw.rect(self.screen, C_PANEL_INNER, (x + 20, total_y, w - 40, 38), border_radius=2)
        ...
        sw_y = y + header_h + 195
        center_x = x + w // 2
        draw_soviet_switch(self.screen, center_x - 90, sw_y, "SALVO", "SGL", self.sw_salvo == 2, self.font_sm)
        draw_soviet_switch(self.screen, center_x, sw_y, "AUTO", "MAN", self.sw_auto, self.font_sm)
        draw_soviet_switch(self.screen, center_x + 90, sw_y, "HV ON", "OFF", self.pwr_hv, self.font_sm)
        
        lamp_y = y + header_h + 280
        draw_soviet_lamp(self.screen, center_x - 90, lamp_y, "READY", C_LAMP_GREEN,
                        self.launchers.launchers[self.launchers.selected]['ready'], self.font_sm)
        draw_soviet_lamp(self.screen, center_x, lamp_y, "INHIBIT", C_LAMP_RED,
                        not self.launchers.can_fire(self.launchers.selected, self.ant_az)[0], self.font_sm)
        draw_soviet_lamp(self.screen, center_x + 90, lamp_y, "FAIL", C_LAMP_ORANGE,
                        len(self.active_missiles) >= 2, self.font_sm)
        
        in_envelope = self.check_launch_envelope() and self.locked
        draw_soviet_lamp(self.screen, x + w//2, y + header_h + 355, "AUTHORIZATION", C_LAMP_YELLOW, in_envelope, self.font_sm)
        
        btn_active = in_envelope and self.launchers.launchers[self.launchers.selected]['ready'] and self.pwr_hv
        draw_soviet_button(self.screen, x + w//2, y + header_h + 450, "FIRE", btn_active, self.font, 35)
        
        prep_hint = self.font_sm.render("F1-F4: PREPARE", True, C_TEXT_DIM)
        self.screen.blit(prep_hint, (x + w//2 - prep_hint.get_width()//2, y + header_h + 500))

    def draw_panel_controls(self, x, y, w, h):
        """电源控制面板"""
        draw_panel(self.screen, (x, y, w, h), "POWER CONTROL", C_PHOSPHOR_GREEN)
        
        header_h = 26
        sw_y = y + header_h + 50
        sw_spacing = 70
        
        draw_soviet_switch(self.screen, x + 40, sw_y, "[1]", "GRID", self.sw_grid, self.font_sm)
        draw_soviet_switch(self.screen, x + 40 + sw_spacing, sw_y, "[2]", "SYS", self.sw_sys, self.font_sm)
        draw_soviet_switch(self.screen, x + 40 + sw_spacing*2, sw_y, "[3]", "HV", self.sw_hv, self.font_sm)
        draw_soviet_switch(self.screen, x + 40 + sw_spacing*3, sw_y, "[4]", "ANT", self.sw_ant, self.font_sm)
        
        lamp_y = y + header_h + 130
        draw_soviet_lamp(self.screen, x + 40, lamp_y, "D", C_LAMP_GREEN, self.pwr_grid, self.font_sm)
        draw_soviet_lamp(self.screen, x + 40 + sw_spacing, lamp_y, "K", C_LAMP_GREEN, self.pwr_sys, self.font_sm)
        draw_soviet_lamp(self.screen, x + 40 + sw_spacing*2, lamp_y, "V", C_LAMP_RED, self.pwr_hv, self.font_sm)
        draw_soviet_lamp(self.screen, x + 40 + sw_spacing*3, lamp_y, "P", C_LAMP_GREEN, self.pwr_ant, self.font_sm)
        
        # 日志终端
        log_y = y + header_h + 190
        log_h = h - header_h - 280
        draw_panel(self.screen, (x + 8, log_y, w - 16, log_h), "SYSTEM LOG", C_TEXT_SECONDARY, C_PANEL_INNER)
        
        for i, msg in enumerate(self.log):
            color = C_TERM_GREEN if i == len(self.log) - 1 else C_TEXT_DIM
            prefix = ">>> " if i == len(self.log) - 1 else "    "
            txt = self.font_sm.render(f"{prefix}{msg}", True, color)
            self.screen.blit(txt, (x + 15, log_y + 28 + i * 16))
    ...
    def draw_panel_status(self, x, y, w, h):
        """状态面板"""
        draw_panel(self.screen, (x, y, w, h), "APPROACH LAUNCH ZONE", C_PHOSPHOR_CYAN)
        
        header_h = 26
        scope_x = x + 15
        scope_y = y + header_h + 12
        scope_w = 250
        scope_h = 250
        
        draw_panel(self.screen, (scope_x, scope_y, scope_w, scope_h), "", C_PANEL_BORDER, C_PANEL_INNER)
        ...
        draw_panel(self.screen, (info_x, info_y, info_w, info_h), "TARGET DATA", C_TEXT_SECONDARY, C_PANEL_INNER)
        ...
        for i, line in enumerate(info_lines):
            txt = self.font_sm.render(line, True, C_TEXT_PRIMARY)
            self.screen.blit(txt, (info_x + 10, info_y + 28 + i * 22))
        ...
        draw_panel(self.screen, (missile_x, missile_y, missile_w, missile_h), "IN FLIGHT", C_TEXT_SECONDARY, C_PANEL_INNER)
        
        missile_title = self.font_sm.render("ACTIVE MISSILES:", True, C_PHOSPHOR_CYAN)
        self.screen.blit(missile_title, (missile_x + 10, missile_y + 28))
        
        for i, m in enumerate(self.active_missiles[:8]):
            md, _, _ = m.get_polar()
            fuel_pct = m.fuel / m.total_fuel * 100
            status = f"M{i+1}: {md/1000:.1f}km F:{fuel_pct:.0f}%"
            txt = self.font_sm.render(status, True, C_TEXT_PRIMARY)
            self.screen.blit(txt, (missile_x + 10, missile_y + 48 + i * 18))
    ...
    def draw_plotting_board(self):
        ...
        draw_panel(self.screen, (board_x, board_y, board_w, board_h), "TACTICAL PLOTTING BOARD", C_PHOSPHOR_GREEN)
        
        header_h = 26
        cx = board_x + board_w // 2
        cy = board_y + header_h + (board_h - header_h) // 2
        scale = ((board_h - header_h) / 2) / 100000
    ...
    def draw_menu(self):
        ...
        pygame.draw.line(sweep, (0, 60, 25, 60), (cx, cy),
                        (cx + 900 * math.sin(rad), cy - 900 * math.cos(rad)), 2)
        ...
        draw_panel(self.screen, (panel_x, panel_y, panel_w, panel_h), "", C_PANEL_BORDER, C_PANEL_BG)
        
        # 标题
        title_y = panel_y + 30
        title1 = self.font_title.render("S-125 NEVA", True, C_TEXT_PRIMARY)
        title2 = self.font_title_sm.render("SA-3 GOA SAM SIMULATOR", True, C_TEXT_SECONDARY)
        
        self.screen.blit(title1, (cx - title1.get_width()//2, title_y))
        self.screen.blit(title2, (cx - title2.get_width()//2, title_y + 55))
        
        # 分隔线
        pygame.draw.line(self.screen, C_PANEL_BORDER, (panel_x + 40, title_y + 95), (panel_x + panel_w - 40, title_y + 95), 1)
        
        # 菜单选项
        menu_font = pygame.font.SysFont("consolas", 24, bold=True)
        options = [
            ("[1] START MISSION", C_PHOSPHOR_GREEN),
            ("[2] OPERATOR MANUAL", C_TEXT_SECONDARY),
            ("[3] EXIT SYSTEM", C_TEXT_DIM)
        ]
        
        start_y = title_y + 120
        for i, (opt, color) in enumerate(options):
            rect = (cx - 220, start_y + i * 85, 440, 60)
            draw_panel(self.screen, rect, "", C_PANEL_BORDER, C_PANEL_INNER)
            
            txt = menu_font.render(opt, True, color)
            self.screen.blit(txt, (cx - txt.get_width()//2, start_y + i * 85 + 17))
        
        # 状态提示
        if int(pygame.time.get_ticks() / 500) % 2:
            status = self.font.render("SYSTEM STANDBY...", True, C_PHOSPHOR_CYAN)
            self.screen.blit(status, (cx - status.get_width()//2, panel_y + panel_h + 30))
        
        # 版本信息
        ver = self.font_sm.render("v2.4.1 // MILITARY TERMINAL INTERFACE", True, C_TEXT_DIM)
    ...
    def draw_briefing(self):
        ...
        draw_panel(self.screen, (doc_x, doc_y, doc_w, doc_h), "MISSION SCENARIO SELECTION", C_PHOSPHOR_CYAN)
        
        header_h = 26
        # 标题
        header = self.font_lg.render("SELECT TARGET SCENARIO", True, C_TEXT_PRIMARY)
        self.screen.blit(header, (doc_x + doc_w//2 - header.get_width()//2, doc_y + header_h + 15))
        
        pygame.draw.line(self.screen, C_PANEL_BORDER, (doc_x + 40, doc_y + header_h + 45), (doc_x + doc_w - 40, doc_y + header_h + 45), 1)
        ...
        y_offset = doc_y + header_h + 65
        for scenario in scenarios:
            box_h = 160
            draw_panel(self.screen, (doc_x + 40, y_offset, doc_w - 80, box_h), "", scenario["color"], C_PANEL_INNER)
            ...
            self.screen.blit(key_txt, (doc_x + 60, y_offset + 18))
            self.screen.blit(name_txt, (doc_x + 120, y_offset + 20))
            ...
            for i, line in enumerate(info_lines):
                txt = self.font.render(line, True, C_TEXT_PRIMARY)
                self.screen.blit(txt, (doc_x + 80, y_offset + 55 + i * 24))
            
            y_offset += box_h + 20
        
        hint = self.font.render("[ESC] Return to Menu", True, C_TEXT_DIM)
        self.screen.blit(hint, (doc_x + doc_w//2 - hint.get_width()//2, doc_y + doc_h - 35))
    ...
    def draw_debriefing(self):
        ...
        header = self.font_title_sm.render("COMBAT RESULTS", True, C_TEXT_PRIMARY)
        ...
        self.screen.blit(result_txt, (SCREEN_WIDTH//2 - result_txt.get_width()//2, 85))
        ...
        draw_panel(self.screen, (panel_x, panel_y, panel_w, panel_h), "ENGAGEMENT DATA", C_PHOSPHOR_CYAN)
        ...
        for i, line in enumerate(data_lines):
            txt = self.font_md.render(line, True, C_TEXT_PRIMARY)
            self.screen.blit(txt, (panel_x + 20, panel_y + 45 + i * 36))
        ...
        draw_panel(self.screen, (map_x, map_y, map_size, map_size), "TRAJECTORY MAP", C_PHOSPHOR_CYAN)
        ...
        # 评价
        eval_y = 500
        draw_panel(self.screen, (panel_x, eval_y, panel_w, 200), "PERFORMANCE RATING", C_PHOSPHOR_CYAN)
        ...
        self.screen.blit(grade_txt, (panel_x + panel_w//2 - grade_txt.get_width()//2, eval_y + 70))
        ...
        blit_text_wrapped(self.screen, comment, (panel_x + 20, eval_y + 120), self.font, C_TEXT_PRIMARY, panel_w - 40)
        
        hint = self.font_lg.render("[ESC] Return to Menu", True, C_PHOSPHOR_CYAN)
    ...
    def draw_manual(self):
        ...
        draw_panel(self.screen, (doc_x, doc_y, doc_w, doc_h), "S-125 NEVA OPERATING MANUAL", C_PHOSPHOR_GREEN)
        
        header = self.font_lg.render("OPERATOR REFERENCE GUIDE", True, C_TEXT_PRIMARY)
        self.screen.blit(header, (doc_x + doc_w//2 - header.get_width()//2, doc_y + 35))
        
        pygame.draw.line(self.screen, C_PANEL_BORDER, (doc_x + 40, doc_y + 65), (doc_x + doc_w - 40, doc_y + 65), 1)
        ...
        for section_title, title_color, lines in sections:
            ...
            for line in lines:
                color = C_PHOSPHOR_AMBER if line.startswith("  -") else C_TEXT_PRIMARY
        ...
        hint = self.font_lg.render("[ESC] Return to Menu", True, C_PHOSPHOR_CYAN)
        self.screen.blit(hint, (doc_x + doc_w//2 - hint.get_width()//2, doc_y + doc_h - 40))
